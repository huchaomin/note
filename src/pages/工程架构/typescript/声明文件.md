...
npm 包的声明文件与全局变量的声明文件(没有export)有很大区别。
...
```ts
// types/foo/index.d.ts
export default enum Directions { // 因为此时你不知道导出的是一个值还是一个类型，所以会报错
...
实际上，`import ... require` 和 `export =` 都是 ts 为了兼容 AMD 规范和 commonjs 规范而创立的新语法，由于并不常用也不推荐使用，所以这里就不详细介绍了，感兴趣的可以看[官方文档](https://www.typescriptlang.org/docs/handbook/modules.html#export--and-import--require)。
...
```ts
// types/foo/index.d.ts
export as namespace foo; // 声明全局变量 foo
export = foo;
...
declare namespace JQuery {
  ...
  function foo(options: JQuery.CustomOptions): string;
}
```
...
随着 ES6 的广泛应用，现在已经 **不建议再使用** ts 中的三斜线指令来声明模块之间的依赖关系了。
